...
struct ObserverFunctor
{
    ...
    void operator() (state_type_2d_ofp &state, double t)
    {
        ...
        //example on how to influence angular velocity of the defect by setting the activity of the active region based on no-crossterms ansatz
...
int main(int argc, char* argv[])
{
    {   openfpm_init(&argc,&argv); //initialize openfpm
        ...
        double kp_dist=std::atof(argv[16]);//0.009;//1.e-3;
        double ki_dist=std::atof(argv[17]);//0.009;//9e-3;
        double outputMin_dist = -2.*spacing;
        double outputMax_dist = 2.*spacing;

        passive_old = ri*spacing;
